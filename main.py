...
if __name__ == '__main__':
    ...
    while True:
        if not gameStarted:
            ...
            spacePos = (monitorSpecs.current_w * 0.05, monitorSpecs.current_h*0.01)
            enter = font.render('Press <ENTER> to generate random seed', False, (180, 180, 180))
            enterPos = (monitorSpecs.current_w * 0.05, monitorSpecs.current_h * 0.04)
            click = font.render('Press <LEFTCLICK> to add a cell', False, (180, 180, 180))
            clickPos = (monitorSpecs.current_w * 0.05, monitorSpecs.current_h * 0.07)
            clear = font.render('Press <RETURN> to clear the grid', False, (180, 180, 180))
            clearPos = (monitorSpecs.current_w * 0.05, monitorSpecs.current_h * 0.1)
            ...
            if key != state_left:  # Button state changed
                state_left = key
                if key < 0:
                    Earth.Entity[int(pyautogui.position().x/cellSize)][int(pyautogui.position().y/cellSize)] = 1
                    Earth.draw()
